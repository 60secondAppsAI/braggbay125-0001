        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalBanners = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalBanners">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Banner</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="BannerId" type="text" placeholder="Enter BannerId" v-model="bannerToAdd.bannerId"></base-input>
  <base-input label="ImageUrl" type="text" placeholder="Enter ImageUrl" v-model="bannerToAdd.imageUrl"></base-input>
  <base-input label="TargetUrl" type="text" placeholder="Enter TargetUrl" v-model="bannerToAdd.targetUrl"></base-input>
  <base-input label="DisplayStartDate" type="text" placeholder="Enter DisplayStartDate" v-model="bannerToAdd.displayStartDate"></base-input>
  <base-input label="DisplayEndDate" type="text" placeholder="Enter DisplayEndDate" v-model="bannerToAdd.displayEndDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="banners" :row-key="record => record.BannerId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <BannerPictureView :banners="banners" />
          </div>
...
import BannerPictureView from './BannerPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'bannerId',
          visible: true,
          scopedSlots: { customRender: 'bannerId' },
          sorter: true
          //sorter: (a, b) => a.bannerId - b.bannerId,
          //sorter: (a, b) => a.bannerId.localeCompare(b.bannerId),
        },
        {
          title: 'Image Url',
		dataIndex: 'imageUrl',
		...
		dataIndex: 'targetUrl',
          ...
          sorter: true
          //sorter: (a, b) => a.targetUrl - b.targetUrl,
		...
		dataIndex: 'displayStartDate',
          visible: true,
          scopedSlots: { customRender: 'displayStartDate' },
          sorter: true
          //sorter: (a, b) => a.displayStartDate - b.displayStartDate,
          //sorter: (a, b) => a.displayStartDate.localeCompare(b.displayStartDate),
        },
        {
          title: 'Display End Date',
		dataIndex: 'displayEndDate',
          visible: true,
          scopedSlots: { customRender: 'displayEndDate' },
          sorter: true
          //sorter: (a, b) => a.displayEndDate - b.displayEndDate,
          //sorter: (a, b) => a.displayEndDate.localeCompare(b.displayEndDate),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'bannerId',           // Column to sort by
  ...
  methods: {
    ...
    async renderBannersTable() {
      ...
      let bannersTableData = [];
      for (let i = 0; i < this.banners.length; i++) {
        bannersTableData.push({
          id: i,
          bannerId: this.banners[i].bannerId,
          year: this.banners[i].year,
          date: this.banners[i].date,
          competitionId: this.banners[i].competitionId,
          bannerId: this.banners[i].bannerId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-banners',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    routingToBannerDetail(id) {
      this.$router.push({ name: 'BannerDetail', params: { bannerId: id.toString() }})
    },
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForBannersChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalBanners = false;

      const currentDate = new Date().getTime();
      this.bannerToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.bannerToAdd);
      console.log(jsonData);
      
      const res = await BannerService.addBanner(jsonData);
